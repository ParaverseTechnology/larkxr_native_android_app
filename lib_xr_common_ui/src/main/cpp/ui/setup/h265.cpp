...
H265Setup::H265Setup(int groupid, QuickConfigSetup* setup): ItemBase(groupid), quick_config_setup_(setup) {
    setTitle(localization::Loader::getResource().ui_setup_advance_use_h265_title);
...
H265Setup::~H265Setup() = default;

void H265Setup::Reset() {
    Set(lark::LK_CONFIG_DEFAULT_USE_H265);
}

void H265Setup::SetAABBPositon(const glm::vec2 &position) {
...
void H265Setup::HandleInput(glm::vec2 *point, int pointCount) {
...
void H265Setup::Enter() {
    FreshData();
}

void H265Setup::Leave() {

}

void H265Setup::OnChange(bool use) {
    if (use != use_) {
...
void H265Setup::Set(bool use) {
    use_ = use;
    lark::XRConfig::use_h265 = use;
    if (use_) {
...
void H265Setup::FreshData() {
    use_ = lark::XRConfig::use_h265;
    if (use_) {
        btn_on_->set_color(COLOR_ACTIVE);
        btn_off_->set_color(COLOR_UN_ACTIVE);
    } else {
