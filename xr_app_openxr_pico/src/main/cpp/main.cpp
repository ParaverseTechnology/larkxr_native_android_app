...
struct AndroidAppState {
    ...
    bool Resumed = false;
    PvrXrApplication* pvrXrApplication = nullptr;
};
...
//        std::shared_ptr<IOpenXrProgram> program = CreateOpenXrProgram(options, platformPlugin, graphicsPlugin);
        std::shared_ptr<OpenxrContext> context = std::make_shared<OpenxrContext>(options, platformPlugin);
        ...
        PvrXrApplication application;
        application.InitGL(context.get());

        appState.pvrXrApplication = &application;
        ...
        while (app->destroyRequested == 0) {
            ...
            for (;;) {
                ...
                const int timeoutMilliseconds =
                    (!appState.Resumed && !context->IsSessionRunning() && app->destroyRequested == 0) ? -1 : 0;
            ...
            context->PollEvents(&exitRenderLoop, &requestRestart);
            if (exitRenderLoop) {
                break;
            }
            if (!context->IsSessionRunning()) {
                continue;
            }
            context->PollActions();
            application.Update();
        ...
        appState.pvrXrApplication = nullptr;
