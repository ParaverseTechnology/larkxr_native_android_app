...
//        }
        ...
        if ((homeValue.isActive == XR_TRUE) && (homeValue.changedSinceLastSync == XR_TRUE) &&
            (homeValue.currentState == XR_TRUE)) {
            Log::Write(Log::Level::Error,
                       Fmt("pico keyevent  homekey click %d",hand));
        }
        ...
        if ((backValue.isActive == XR_TRUE) && (backValue.currentState != XR_TRUE) && (backValue.changedSinceLastSync == XR_TRUE)) {
            float enbale6Dof;
            GetConfig(ENABLE_6DOF, &enbale6Dof);
            Log::Write(Log::Level::Info,
                       Fmt("pico  pfnXrGetConfigPICO  enbale6Dof %f",enbale6Dof));
            if((int)enbale6Dof == 1) {
                ResetSensor(XR_RESET_ALL);
            } else {
                ResetSensor(XR_RESET_ORIENTATION);
            }
        }
        ...
        if ((sideValue.isActive == XR_TRUE) && (sideValue.changedSinceLastSync == XR_TRUE) &&
            (sideValue.currentState == XR_TRUE)) {
            Log::Write(Log::Level::Error,
                       Fmt("pico keyevent  sidekey click %d",hand));
        }
//            Log::Write(Log::Level::Error,
